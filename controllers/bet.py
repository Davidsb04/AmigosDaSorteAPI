...
def check_bet(group_id, match_id):
    if 'user_id' in session:
        user_id = session['user_id']
        bet_ref = db.collection('groups').document(group_id).collection('bets').where(
            'match_id', '==', match_id).where('user_id', '==', user_id).stream()
        ...
        if res.status == 200:
            ...
            bet_doc_ref = db.collection('groups').document(
                group_id).collection('bets').document(bets[0].id)
