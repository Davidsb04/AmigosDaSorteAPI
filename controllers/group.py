    ...
    if 'user_id' in session:
        ...
        try:
            ...
            db.collection('groups').document(
                group_id).update({'group_id': group_id})
...
def get_group(group_name):
    if 'user_id' in session:
        docs = db.collection('groups').where(
            'group_name', '==', group_name).stream()
...
def user_bets(group_id):
    if 'user_id' in session:
        user_id = session.get('user_id')
        bets_ref = db.collection('groups').document(group_id).collection(
            'bets').where('user_id', '==', user_id).stream()
